...
function fill_product($pdo, $idp){
    ...
    $id=$_GET['id'];
$select=$pdo->prepare("select * from tbl_outbound where invoice_id =$id");
$select->execute();
...
$select=$pdo->prepare("select * from tbl_outbound_details where invoice_id =$id");
$select->execute();
...
//fin codigo para mostrar detalles del pedido
 ...
 if(isset($_POST['btnupdateorder'])){
     ...
     */
     ...
     $delete_invoice_details=$pdo->prepare("delete from tbl_outbound_details where invoice_id=$id");
    
        $delete_invoice_details->execute(); 
     /** (4) ESCRIBIR UN UPDATE PARA datos de tbl_invoice  */

     $update_invoice=$pdo->prepare("update tbl_outbound set customer_name=:cust,order_date=:orderdate,subtotal=:stotal,tax=:tax,discount=:disc,total=:total,paid=:paid,due=:due,payment_type=:ptype where invoice_id=$id");
      ...
      <!--------------------------
        ...
        <div class="box box-warning">
            ...
            <div class="box-header with-border">
              <h3 class="box-title">Editar Orden Saliente</h3>
            </div>
            <!-- /.box-header -->
            <!-- form start -->
