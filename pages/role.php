<?php
  ...
  if(isset($_POST['btnsave'])){
    $role=$_POST['txtrole'];
    if(empty($role)){ //Si esta vacia, se mostrara un mensaje de error
        $error='
    ...
    if(!isset($error)){
        $insert=$pdo->prepare("insert into tbl_role(role) values (:role)");
...
if(isset($_POST['btnupdate'])){
    ...
    if(empty($role)){ //Si esta vacia, se mostrara un mensaje de error
        ...
        <script type="text/javascript">
            ...
            swal({
                title: "Error!",
                text: "El campo esta vacio, por favor ingresar Rol",
                icon: "error",
                button: "OK",
            });
            });
    ...
    if(!isset($errorupdate)){
        $update=$pdo->prepare("update tbl_role set role=:role where idrole=".$id);//podemos pasar el placeholder
...
if(isset($_POST['btndelete'])){
    $delete=$pdo->prepare("delete from tbl_role where idrole=".$_POST['btndelete']);//podemos pasar el placeholder
    if($delete->execute()){
        echo '<script type="text/javascript">
        jQuery(function validation(){
          swal({
            ...
            <!-- form start -->
              ...
              <form role="form" action="" method="post"><!--Formulario para editar-->
                  ...
                  if(isset($_POST['btnedit'])){ //si presiono editar  las categorias, se mostrara otro formulario y escondera el actual
                    $select=$pdo->prepare("select * from tbl_role where idrole=".$_POST['btnedit']);//Obtengo los datos
                    ...
                    <input type="hidden" class="form-control" value="'.$row->idrole.'" name="txtidrole" placeholder="Ingresar Rol">
                    
                    <input type="text" class="form-control"  value="'.$row->role.'" name="txtrole" placeholder="Ingresar Rol">
                    ...
                    <div class="form-group">
                    <label >Rol</label>
                    <input type="text" class="form-control" name="txtrole" placeholder="Ingresar Rol">
                    </div>
                            ...
                            <?php
                             ...
                             while($row=$select->fetch(PDO::FETCH_OBJ)){//Recorro los registros los valores
                                 echo'<tr>
                                    ...
                                    <td>'.$row->role.'</td>
                                    <td>
                                    <button type="submit" value="'.$row->idrole.'" class="btn btn-success" name="btnedit">Editar</button>
                                    </td>
                                    <td>
                                    <button type="submit" value="'.$row->idrole.'" class="btn btn-danger" name="btndelete">Eliminar</button>
                                    </td>
                                </tr>';
                             }
